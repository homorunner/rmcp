...
impl StreamableHttpClient for reqwest::Client {
    ...
    async fn get_stream(
        ...
        if let Some(auth_header) = auth_token {
            request_builder = request_builder.bearer_auth(auth_header.clone());
            request_builder = request_builder.header("access_token", auth_header);
        }
    ...
    async fn post_message(
        ...
        if let Some(auth_header) = auth_token {
            request = request.bearer_auth(auth_header.clone());
            request = request.header("access_token", auth_header);
        }
        ...
        match content_type {
            ...
            _ => {
                ...
                tracing::error!("unexpected content type: {:?} {}", content_type, response.bytes().unwrap());
